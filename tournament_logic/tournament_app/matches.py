...
def new_round(user):
    try:
        ...
        return None
    plyr = get_player(user_prf)
    trn =  plyr.tournament
    # create_matches(request, trn)
...
def get_matche(matches, user):
    for matche in matches:
        if matche.player1.profile.user == user:
            return matche
        if matche.player2.profile.user == user:
            return matche
